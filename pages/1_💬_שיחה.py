...
אם לא היום: <!--MEAL:{{"type":"סוג הארוחה","description":"תיאור קצר","date":"YYYY-MM-DD"}}-->
...
# ── DB helpers ─────────────────────────────────────────────────────────────────
def save_meal(raw_input: str, meal_type: str, description: str, meal_date: str | None = None):
    db.insert("meal_log", {
...
def show_daily_summary(for_date: str | None = None):
    target = for_date or date.today().isoformat()
    meals = db.select("meal_log", filters={"meal_date": target}, order="created_at.asc")
    if not meals:
        return
    label = "היום" if target == date.today().isoformat() else target
    st.markdown(f"**📋 סיכום ארוחות {label}:**")
    rows = []
    for m in meals:
...
if active_photo and not st.session_state.get("_last_photo") == active_photo.name + str(active_photo.size):
    ...
    with st.chat_message("assistant"):
        ...
        if meal_data:
            ...
            save_meal("[תמונה]", meal_data.get("type", "ארוחה"), meal_data.get("description", ""), meal_date)
            date_label = f" ({meal_date})" if meal_date else ""
            clean_reply += f"\n\n*✅ הארוחה נרשמה ביומן{date_label}*"

        if blood_sugar_data:
            bs_date = blood_sugar_data.get("date")
            save_blood_sugar(float(blood_sugar_data.get("value", 0)), blood_sugar_data.get("reading_time"), bs_date)
            date_label = f" ({bs_date})" if bs_date else ""
            clean_reply += f"\n\n*✅ סוכר בדם נרשם: {blood_sugar_data.get('value')} mg/dL{date_label}*"
        ...
        if meal_data:
            show_daily_summary(for_date=meal_data.get("date"))
...
if prompt := st.chat_input("כתבי הודעה..."):
    ...
    with st.chat_message("assistant"):
        ...
        if meal_data:
            meal_date = meal_data.get("date")
            save_meal(prompt, meal_data.get("type", "ארוחה"), meal_data.get("description", ""), meal_date)
            date_label = f" ({meal_date})" if meal_date else ""
            clean_reply += f"\n\n*✅ הארוחה נרשמה ביומן{date_label}*"

        if weight_data:
            w_date = weight_data.get("date")
            save_weight(float(weight_data.get("kg", 0)), w_date)
            date_label = f" ({w_date})" if w_date else ""
            clean_reply += f"\n\n*✅ המשקל נרשם: {weight_data.get('kg')} ק״ג{date_label}*"
